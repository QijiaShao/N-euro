...
class TimeSeriesDataSet(Dataset):
    ...
    * etc.
    ...
    def __init__(
        ...
        # target normalizer
        # self._set_target_normalizer(qs_added_gt)
        ...
        self._validate_data(qs_added_gt)
        ...
        if min_prediction_idx is not None:
            ...
            qs_added_gt = qs_added_gt[lambda x: x[self.time_idx] >= self.min_prediction_idx - self.max_encoder_length - self.max_lag]
        data = data.sort_values(self.group_ids + [self.time_idx])
        qs_added_gt = qs_added_gt.sort_values(self.group_ids + [self.time_idx])

        # preprocess data
        data = self._preprocess_data(data)
        qs_added_gt = self._preprocess_data(qs_added_gt)
        for target in self.target_names:
        ...
        self.qs_added_gt = self._data_to_tensors(qs_added_gt)
    ...
    def get_parameters(self) -> Dict[str, Any]:
        ...
        kwargs = {
            ...
            if name not in ["data", "self", "qs_added_gt"]
        }
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        # print(type(data_cont)); print(data_cont)
        ...
        target_raw = [d[index.index_start : index.index_end + 1].clone() for d in self.data["target"]]  # original code
        target = [d[index.index_start : index.index_end + 1].clone() for d in self.qs_added_gt["target"]]
        
        # print(target[0], [d[index.index_start : index.index_end + 1].clone() for d in self.data["target"]][0]) # testing the setup
        ...
        for idx, target_normalizer in enumerate(self.target_normalizers):
            if isinstance(target_normalizer, EncoderNormalizer):
                ...
                if target_name in self.reals:
                    ...
                    data_cont[:, self.reals.index(target_name)] = target_normalizer.transform(target_raw[idx])
                if self.add_target_scales:
        ...
        if self.multi_target:
            encoder_target = [t[:encoder_length] for t in target_raw]
            target = [t[encoder_length:] for t in target]
        else:
            encoder_target = target_raw[0][:encoder_length]
